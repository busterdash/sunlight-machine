...
impl SMDReader {
    ...
    pub fn new(smd_path: &str) -> Self {
        ...
        let mut vbuffer: [Vertex; 3] = [Vertex::default(); 3];
    ...
    pub fn get_max_coordinate(&self) -> f64 {
        ...
        for i in 0 .. self.get_triangle_count() { 
            for j in 0 .. 3 {
                if self.tri_list[i].v[j].x > max {
                    max = self.tri_list[i].v[j].x;
                }
                
                if self.tri_list[i].v[j].y > max {
                    max = self.tri_list[i].v[j].y;
                }
                
                if self.tri_list[i].v[j].z > max {
                    max = self.tri_list[i].v[j].z;
                }
            }
        }
